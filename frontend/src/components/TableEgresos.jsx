...
import {
  fetchGetAllEgresos,
  fetchPostEgreso,
} from "../http/fetchEgreso"; // Importamos las funciones del backend
...
export const TableEgresos = () => {
  ...
  const cargarEgresos = async () => {
    ...
    try {
      const data = await fetchGetAllEgresos(cuentaId); // Llamada con el filtro
      setEgresos(data);
    } catch (error) {
  ...
  const handleAgregarEgreso = async (formData) => {
    setButton(true);
    try {
      const nuevoEgreso = {
      ...
      await fetchPostEgreso(nuevoEgreso);
      setModalForm(false);
      reset();
      cargarEgresos(); // Actualizar la lista después de agregar
    } catch (error) {
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  ...
                  <tbody>
                    {egresos.map((egreso) => (
                      <tr key={egreso.id_egreso} className="border-b dark:border-gray-700">
                        <th
                          scope="row"
                        ...
                        <td className="px-4 py-3">{egreso.monto}</td>
                ...
                <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                  ...
                  <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                      ...
                      <form
                        onSubmit={handleSubmit(handleAgregarEgreso)}
